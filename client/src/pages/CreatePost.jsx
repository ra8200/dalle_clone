...
const CreatePost = () => {
  ...
  const generatingImage = async () => {
    if(form.prompt) {
      try {
        ...
        const response = await fetch('https://dall-e-clone-4fga.onrender.com/api/v1/dalle', {
          method: 'POST',
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if(form.name && form.photo) {
      ...
      try {
        const response = await fetch('https://dall-e-clone-4fga.onrender.com/api/v1/posts', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
